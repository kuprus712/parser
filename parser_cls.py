...
import time
import re
from database import save_ad_to_db, get_ads_from_db, close_connection
...
from telegram.ext import ApplicationBuilder, CommandHandler, MessageHandler, CallbackContext, filters, Update

def start(update: Update, context: CallbackContext):
    welcome_text = (
        "Привет! Я помогу найти квартиру без комиссии.\n\n"
        "Чтобы пользоваться ботом:\n"
        "1️⃣ Оформи подписку.\n"
        "2️⃣ Выставь фильтры (город, цена, количество комнат).\n"
        "3️⃣ Получай подходящие объявления.\n\n"
        "Для подписки введи /subscribe."
    )
    await update.message.reply_text(welcome_text)

application = ApplicationBuilder().token("7565773233:AAFStJBOaUNtcXGfji9X1xY5-z3vUvMUca4").build()
...
class AvitoParse:
    ...
    """
    def __init__(self,
                 url: "https://www.avito.ru/kazan/kvartiry/sdam/na_dlitelnyy_srok/bez_komissii-ASgBAgICA0SSA8gQ8AeQUp74DgI?context=H4sIAAAAAAAA_wEjANz_YToxOntzOjg6ImZyb21QYWdlIjtzOjc6ImNhdGFsb2ciO312FITcIwAAAA&district=784&f=ASgBAQICA0SSA8gQ8AeQUp74DgIBQMwItJhZoqwBoKwBnqwBnKwBllmUWZJZkFmMWY5Z&user=1",
                 keysword_list: list,
                 keysword_black_list: list,
                 count: int = 10,
                 tg_token: str = "7565773233:AAFStJBOaUNtcXGfji9X1xY5-z3vUvMUca4",
                 max_price: int = 500000,
                 min_price: int = 15000,
                 geo: str = None,
                 debug_mode: int = 0,
                 need_more_info: int = 1,
                 proxy: str = "NyxkEu:eB4MUYUh4eke@fproxy.site:13844",
                 proxy_change_url: str = "https://changeip.mobileproxy.space/?proxy_key=b5545da96791e0709d19427fafcaf294",
                 stop_event=None,
                 max_views: int = 400,
                 fast_speed: int = 1
                 ):
